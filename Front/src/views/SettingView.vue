...
export default {
  ...
  methods: {
    ...
    handleInput() {
      ...
      if (this.otpCode) this.otpCode = this.otpCode.replace(/\D/g, "");
    },
...
<template>
  ...
  <div v-else
    v-if="this.isError"
    class="flex ml-20 lg:ml-24 items-center justify-center h-screen dark:bg-slate-800 p-10"
  >
    <div class="text-center">
      ...
      <div class="flex gap-5 items-center justify-center w-full">
        <button
          ...
          class="mt-8 text-blue-500 hover:underline text-lg"
        >
          Go to Home
        </button>
        <button
  ...
  <div
    v-else
    class="m-auto flex items-center justify-center h-screen ml-20 lg:ml-24 dark:bg-slate-800"
  >
    <!-- Info Card -->
    <div
    ...
    >
      ...
      <label class="relative w-36 h-36 bg-gray-300 rounded-full shadow-lg cursor-pointer">
        <img
          ...
          :src="this.avatar"
          alt="Avatar"
          class="aspect-square object-cover rounded-full w-36 h-36 opacity-70"
        />
        <Icon
    ...
    >
      <div
        class="flex w-full justify-start items-center pl-10 pt-7 font-bold text-2xl dark:text-white"
      >
        Change your Name:
      </div>
      ...
      <p v-if="this.newName.length > 12" class="text-red-500 pb-2">3aya9tiiiii !!</p>
    ...
    >
      <img src="http://localhost:3000/api/2fa/generate" alt="" class="w-52 h-52 md:w-64 md:h-64" />
      <p
    ...
    >
      <h2
        class="flex w-full justify-start items-center py-5 px-10 font-light text-xl text-gray-500"
      >
