...
export class ChatRoomsGateway {
  ...
  async handleConnection(socket: Socket) {
    try {
      ...
      const message = await this.chatRoomsService.messagesNotification(
        payload.sub,
      );
      notifications.push({
        type: "info",
        message: `You Got ${message} new Messages`,
      });
  ...
  async remove(@MessageBody() body, @Req() req) {
    ...
    try {
      if (id && typeof id === "number") {
        const chatroom = await this.chatRoomsService.remove(
          id,
          payload,
          clients,
        );
        ...
        client.emit("Notification", {
          type: "info",
          message: `${chatroom.affected} has been deleted.`,
        });
      }
    } catch (e) {
      const client = clients.get(payload.sub);
      ...
      if (
        ...
        if (updatedMember.role == "admin")
          await this.chatRoomsService.newChatMessage(
            payload.sub,
            chatId,
            `${updatedMember.user.username} is an ${updatedMember.role} now.`,
            "notification",
            clients,
            "role",
          );
          ...
          await this.chatRoomsService.newChatMessage(
            payload.sub,
            chatId,
            `${updatedMember.user.username} is a normal ${updatedMember.role} now.`,
            "notification",
            clients,
            "role",
          );
    ...
    try {
      if (
        ...
        await this.chatRoomsService.newChatMessage(
          payload.sub,
          chatId,
          `${updatedMember.user.username} is ${updatedMember.userStatus}.`,
          "notification",
          clients,
          "status",
        );
  ...
  async kickMember(@MessageBody() body, @Req() req) {
    ...
    const { memberId, chatId } = body;
    const payload = verifyToken(req.handshake.headers.cookie);
    try {
      if (typeof memberId === "number" && typeof chatId === "number") {
        const kick = await this.chatRoomsService.kickMemberFromChat(
          memberId,
          chatId,
          payload,
          clients,
        ...
        if (memberId != payload.sub)
          await this.chatRoomsService.newChatMessage(
            payload.sub,
            chatId,
            `${kick.user.username} was kicked from chat.`,
            "notification",
            clients,
            "kick",
          );
    ...
    try {
      if (typeof chatId === "number") {
        ...
        if (!chat) throw new NotFoundException();
          ...
          chat.privacy == "public" ||
          (chat.privacy == "protected" && checked)
        ) {
          const member = await this.chatRoomsService.addMemberToChat(
            chatId,
            payload.sub,
          );
          await this.chatRoomsService.newChatMessage(
            chat.owner,
            chatId,
            `${member.username} joined the chat.`,
            "notification",
            clients,
            "joined",
          );
        } else if (!checked) throw new UnauthorizedException();
      ...
      if (typeof id === "string") {
        ...
        if (invitation) {
          await this.chatRoomsService.newChatMessage(
            ...
            `${invitation.toUser.username} joined the chat.`,
            "notification",
            clients,
            "joined",
          );
